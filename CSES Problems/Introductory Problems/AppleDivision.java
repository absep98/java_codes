...
public class AppleDivision {
    ...
    public static long solve(int indx, long asum, long bsum, int[] arr) {
        if (indx == arr.length) {
            return Math.abs(asum - bsum);
        }
        long ch1 = solve(indx + 1, asum + arr[indx], bsum, arr);
        long ch2 = solve(indx + 1, asum, bsum + arr[indx], arr);
